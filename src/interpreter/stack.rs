...
impl<'txin> Element<'txin> {
    ...
    /// Convenience method to convert push element to a Bitcoin script.
    pub(super) fn to_script(self) -> bitcoin::ScriptBuf {
        bitcoin::ScriptBuf::from(self.as_push().to_owned())
    }
}
